...
export function useVenueDetail() {
  ...
  const galleryImages = computed(() => {
    ...
    return uniqueImages
      .map((item) => resolveVenueAssetUrl(item))
      .filter(Boolean);
  });

  // 推荐标记、字典文案和后台标签统一整理成展示标签
  const displayTags = computed(() =>
    buildVenueDisplayTags(venue.value, {
  ...
  async function initializeVenueDetailPage() {
    await Promise.all([loadVenueDictionaries(), loadVenueDetail()]);
  }
